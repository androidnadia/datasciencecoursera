...
```{r}
fit <- lm(median ~ major_category - 1, college_tmp)
round(summary(fit)$coef, 3)
```


As expected, the estimated values are the mean of the median income for each major category. Since all the different variables are compared to 0, the p-value is then statistically significant for each category. The plot of the residuals doesn't show any pattern and looks normally distributed. Note: In the plot, there is a clear outlier.  
...
Our basic analysis shows that there is a difference between some major and some other majors. There is a significant difference (p-value < 0.05) btween **Business** compared to **Arts**, **Computers and Mathematics**, **Education**, **Engineering**, **Humanities & Liberal Arts** and **Social Science**. 
